...
#rawfile = pd.read_csv("/Users/Guillaume/Desktop/20180908204417_STROM1.csv",sep=";", engine='python')
#df = calibration(rawfile,12)
#a = df.getcalib(7)
#print a
#dg = df.applycalib(a[0][0],a[1][0],a[2][0])
#df1 = initialize(dg)
#df1.clean_parameter('NB_SATELLITE',1,20)
#df1.clean_parameter('PUMP_FLUX',-1,1)
#df1.clean_flux('R-squared',0,1)
#df1.clean_time()
#df1.split('/Users/Guillaume/Desktop',60)
...
#rawfile2 = pd.read_csv("/Users/Guillaume/Desktop/SpatialSerie_45.csv", sep=";", engine='python')
#dq = processing(rawfile2,0,14)
...
class analysis:
    ...
    def graph_combi(self,par,dico,lar,lon):
        ...
        for key,c in zip(dico.keys(),color):
            ...
            if len(dico[key])>1:
                ...
                st = 0
            ax.annotate(str(key)+' ('+str(perc)+'%): Mean='+str(av)+'+/-'+str(st), xy=(quantiles[int(index+len(dico[key])/2)], values[int(index+len(dico[key])/2)]), xytext=(quantiles[int(index+len(dico[key])/2)]+0.5, values[int(index+len(dico[key])/2)]),color=c) # Plot the name of the key and the mean+stdev directly on the plot
        ...
        plt.xticks(ticks_quan,ticks_perc) #assign new ticks
    ...
    def distribution(self,par,lar,lon):
        ...
        plt.text(0.05, 0.95, 'Anderson-Darling normality test ($A^{2}$): '+str(anderson)+'\nIf $A^{2}$>0.752 hypothesis of normality rejected at 95% of significance level', transform=ax.transAxes, fontsize=10, verticalalignment='top', bbox=props) # Print the result of the test
    ...
    def plot_campaign_pop(self,par,dictiopopu,coordonnee,lar,lon):
        ...
        for keyf,c in zip(dicofinal.keys(),colors): 
            ax.scatter(dicofinal[keyf][0], dicofinal[keyf][1], edgecolors='black',linewidth=1,color=c, marker='o', s=50, label=str(keyf)+': from '+str(round(np.min(dictiopopu[keyf]),2))+' to '+str(round(np.max(dictiopopu[keyf]),2))) 
        handles, labels = ax.get_legend_handles_labels()
        ax.legend(reversed(handles), reversed(labels), loc='lower left',scatterpoints=1,fontsize=10)        
        ax.ticklabel_format(useOffset=False)
        plt.xticks(rotation=70)
        try:
            plt.imshow(img, zorder=0, extent=coordonnee) 
...
#findat = analysis(dr,'CO2_10')
#findat.lin_reg('P_in',5,14)
#a = findat.correlation()
#a = findat.statistics()
#print a
#findat.distribution('CO2_10',300,300)
#findat.number_pop()
#dicopop = findat.popul(2)
#findat.graph_combi(dicopop)
#corners = findat.download_map(15)
#findat.plot_campaign_pop('CO2_10',dicopop,corners,300,300)
#findat.plot_campaign_all('CO2_100',corners,300,300)
